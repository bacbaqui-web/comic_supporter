...
export function initBookmarks() {
  ...
  const getTabs = () => {
    const tabs =
      Array.isArray(window.__bookmarkTabList) && window.__bookmarkTabList.length
  ...
  const renderTabs = () => {
    ...
    const tabs = getTabs();
    if (!tabs.some((t) => t.id === window.__bookmarkActiveTabId))
      window.__bookmarkActiveTabId = tabs[0]?.id || 'default';
  ...
  const backupBookmarkTab = (tabId) => {
    ...
    const rows = (window.imageBookmarks || []).filter(
